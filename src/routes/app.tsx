...
import { consumeDepositNotification } from "@/lib/pix.functions";
...
function AppLayout() {
  ...
  const check = useServerFn(consumeDepositNotification);
  const [popup, setPopup] = useState<{ amountCents: number } | null>(null);
  ...
  useEffect(() => {
    if (!ready) return;
    const tick = () => check().then((r) => {
      if (r.newDeposit) setPopup({ amountCents: r.newDeposit.amountCents });
    }).catch(() => {});
    tick();
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <PaymentSuccessDialog
        open={!!popup}
        amountCents={popup?.amountCents ?? 0}
        onClose={() => { const p = popup; setPopup(null); if (p) navigate({ to: "/app/tasks" }); }}
      />
    </div>
  );
}
