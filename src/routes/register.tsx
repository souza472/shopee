...
import { Smartphone, Lock, User, IdCard, Ticket } from "lucide-react";
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const { error } = await supabase.auth.signUp({
        ...
        options: {
          data: { phone: digits, name: name.trim(), document: doc, referred_by: referral.trim().toUpperCase() || null },
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center px-6 pt-8 pb-6">
        <div className="flex flex-col items-center mb-4">
          ...
          <div className="mt-2 px-3 py-1 rounded-full bg-emerald-100 text-emerald-700 text-xs font-medium">🎁 Ganhe R$ 5,00 de bônus ao cadastrar!</div>
        </div>
        ...
        <form onSubmit={onSubmit} className="w-full max-w-sm space-y-4">
          ...
          <Field icon={<IdCard className="text-primary" size={22} />}>
            <input value={document} onChange={(e) => setDocument(e.target.value)} inputMode="numeric" placeholder="CPF ou CNPJ" className="flex-1 bg-transparent outline-none" required />
          </Field>
