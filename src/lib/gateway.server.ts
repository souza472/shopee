...
export async function loadGatewayConfig() {
  ...
  if (!data?.api_key_encrypted) throw new Error("Pagamento não aprovado. entre contato com seu gerente!");
...
export async function pixIn(payload: PixInPayload) {
  ...
  if (!res.ok) {
    throw new Error(`Erro de Pagamento ${res.status}: ${json?.message ?? json?.error ?? text.slice(0, 200)}`);
  }
  return json;
}
